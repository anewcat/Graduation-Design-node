...
router.post('/', async (ctx,next) => {
    ...
    let a = await query(`select count(*) as total from petInform`)
	...
	if (page_num == 1) {
        if(query_data ==''){
            // 没有指定搜索内容的时候
            let a1 = await query(`select * from petInform limit 0,${page_size}`)
            ...
            let a1 = await query(`select * from petInform where petName = '${query_data}' limit 0,${page_size}`)
            let result1 = JSON.parse(JSON.stringify(a1))
            // console.log(result1,"resultresultresult123123123")
            let a2 = await query(`select count(*) as total from petInform where petName = '${query_data}'`)
		...
		let sql = "select * from petInform limit ?,?"
        ...
        let a1 = await query(`select * from petInform where petName = '${query_data}' limit ${parmasA},${parmasB}`)
        let result1 = JSON.parse(JSON.stringify(a1))
		let a2 = await query(`select count(*) as total from petInform where petName = '${query_data}'`)
        let result2 = JSON.parse(JSON.stringify(a2))
